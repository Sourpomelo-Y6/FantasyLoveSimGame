...
- `Ending`: エンディング用スチル
- `Training`: 訓練選択時、訓練進行後、主人公・ヒロイン・双方同時のLP消費時に表示するスチル
...
訓練画像の状態、命名、`training_images_export.json`、Unity側の表示優先順位は `Docs/Extra_FantasyLoveSimAssetTool/TrainingImagePlan.md` を正とする。Unity側の最近の機能をToolへ反映する優先順位と互換方針は `Docs/Extra_FantasyLoveSimAssetTool/CurrentFeatureSyncPlan.md` を参照する。
...
予定イベントはヒロイン別 `ActionData` ではなく、ゲーム本体が `ScheduleType` から参照する `ScheduledEventData` として扱う。
実行時はヒロイン別 `ScheduledEvents` を優先し、該当 `ScheduleType` がない場合だけ共通 `Assets/Resources/ScheduledEvents/` をフォールバックとして使う。
...
`preparationMessage` は朝の予定表示、`eventMessage` は予定実行時の本文に入る。未指定の場合は `lines[0]` を準備文、`lines[1...]` を実行本文として補完する。
`imageAssetIds[0]` は `stillId` と `stillSprite` に解決する。
`Data/action_reactions_export.json` が存在する場合は、`conditions.actionId` に対応する `Assets/Resources/Heroines/<HeroineId>/Actions/<Action>.asset` の `ActionData.reactions` を生成、更新する。
既存 `ActionData` がある場合はその action の reactions を JSON 由来で置き換え、ない場合は最小の `ActionData` を作成する。
`lines[0]` は `ActionReactionData.resultMessage`、`imageAssetIds[0]` は `stillId` と `stillSprite`、`conditions.weather` / `season` / `timeOfDay` は条件に変換する。
`Data/endings_export.json` が存在する場合は、`Assets/Resources/Heroines/<HeroineId>/Endings/<EndingId>.asset` を生成、更新する。
`lines[]` は改行で結合して `EndingData.message` に入れ、`imageAssetIds[0]` は `EndingData.stillSprite` に解決する。
prompt JSON の import は次段階で対応する。
Importer は `HeroineImportReport` で copied images、catalog assets、layers、conversations、game events、scheduled events、action reactions、endings、warning 件数を集計し、完了時に Console summary と `EditorUtility.DisplayDialog` で結果を表示する。
...
| `Data/scheduled_events_export.json` | 予定イベント、お出かけ、デート本文データの生成、更新 |
| `Data/action_reactions_export.json` | 行動反応本文データの生成、更新 |
| `Data/endings_export.json` | エンディング本文データの生成、更新 |
| `Data/training_images_export.json` | 訓練画像の状態と `assetId` の対応を生成、更新 |
...
ゲーム開始、日開始、場所イベントなど、イベント単位で発生する本文を扱う。
...
翌日予定の外出、デート、家で過ごす予定など、`ScheduleType` から実行される本文を扱う。
通常行動の `Walk` とは別に、Unity 側の対応先は `ScheduledEventData` とする。
  ...
  "items": [
    {
      "id": "AutoWalkShopping",
      "title": "街への外出",
      "category": "SoloShopping",
      "conditions": {
        "scheduleType": "SoloShopping",
        "actionId": "AutoWalkShopping",
        "triggerTimeSlot": "Noon",
        "outfitPromptMode": "Conditional",
        "eventSpeakerType": "Heroine",
        "affectionChange": 1
      },
      "preparationMessage": "今日は昼に買い物へ行く予定です。",
      "eventMessage": "街を歩きながら、少しだけ距離が近づいた気がします。",
      "imageAssetIds": [
        "Walk_Shopping_01"
      ],
      "priority": 0,
