...
- AssetToolの `制作状況` タブは、現在選択中のヒロインについて基本情報、戦闘メッセージ、訓練画像、訓練セリフ、会話、表情、衣装、戦闘スキル、スキルツリー、イベント、Export準備を既存データから `○ / △ / ×` で都度集計する。訓練セリフは登録済み訓練ごとの5状態について枠・重複・本文を確認し、詳細クリックで対応Assetとセリフ候補を選択する。イベントのID・本文・条件・各参照と、Accepted画像の実ファイルを含むExport前の読み取り専用検査も個別に確認できる。未完成のみ表示と再集計に対応する
