...
訓練スキルの有効状態管理は実装済み。主人公は `SaveData.activePlayerTrainingSkillIds`、ヒロインは `SaveData.heroineTrainingSkillActivations` にヒロイン ID 別で保存する。取得済み訓練ノードでは `SkillTreePanel` の既存ボタンを「有効にする／無効にする」として使い、有効数に上限はない。ロード時は存在しない・未取得・`SkillData` 側で無効なスキル ID を除去する。セーブバージョン15以前は取得済み訓練スキルを自動的に有効化し、16以降は空または無効の選択も維持する。初期ノードは主人公「深呼吸」、DefaultHeroine / TestHeroine「呼吸合わせ」で、総訓練回数1回と1 SPを条件にする。現時点では効果計算へ未接続で、有効状態だけを管理する。
