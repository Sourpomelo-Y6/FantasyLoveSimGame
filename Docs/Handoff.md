...
スケジュールテンプレート機能も実装済み。`ScheduleTemplateManager` は7日・30日の予定種別を `Application.persistentDataPath/schedule_templates.json` へ保存し、セーブスロット間で共有する。一覧、新規保存、ID指定の上書き・削除、適用前プレビュー、選択日からの一括適用を提供する。状態は保存せず、適用時は予定を `Planned` として扱う。既存予定の上書き可否を指定でき、過去、編集可能範囲外、実行済みの日はスキップする。`ScheduleTemplatePanelController` は配置済みのUIを名前から自動検出し、一覧生成、選択、新規保存、確認付き上書き・削除、プレビュー後の適用、結果表示、カレンダー更新を制御する。`SchedulePanel` は `OpenTemplateButton` から現在の選択日を渡す。ControllerがSceneに未配置でも `ScheduleTemplateManager` と同じGameObjectへ実行時に追加する。
