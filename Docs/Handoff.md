...
訓練画像の自動切替は未実装で、設計を `Docs/Extra_FantasyLoveSimAssetTool/TrainingImagePlan.md` に記録済み。訓練選択時は現在の訓練画面で `elapsedSteps == 0` なら開始前、1以上なら進行後の画像を使う。ステップごとのLP差分から主人公のみ、ヒロインのみ、双方同時の3状態を判定し、同時状態を優先する。画像はヒロイン別 `HeroineTrainingImageData` に分離し、既存 `heroineImage` を使い、未設定時は現在画像を維持する。初期素材は3訓練の開始前・進行後6枚と共通LP画像3枚の合計9枚を想定する。
...
訓練メニューの追加解放は実装を後回しにする。将来は熟練度や日数による自動段階解放ではなく、スキルツリーで取得する「訓練解放ノード」として実装する。`SkillTreeNodeData.unlockedTrainingIds` のような対象 ID リストを追加し、`SkillData` を持たない解放効果専用ノードも取得可能にする。解放状態は新しいセーブ項目へ二重保存せず、取得済みノード ID から導出する。ヒロインノードは `targetHeroineId` と現在ヒロインが一致する場合だけ有効とし、初期訓練は無条件で利用可能な状態を維持する。`TrainingPanel` は未解放訓練を非表示または無効表示し、ツリー詳細には解放する訓練名を表示する。
実装時の Validator は存在しない・重複した訓練 ID を検出する。解放ノード自身で初めて利用可能になる訓練の熟練度・実績を同じノードの取得条件にすると到達不能になるため、前提ノードを含む解放経路と条件の循環も警告する。削除済みノードや訓練 ID は安全に無視し、画面を例外停止させない。
