...
- AssetToolの `制作状況` タブは、現在選択中のヒロインについて基本情報、戦闘メッセージ、訓練画像、会話、表情、衣装を既存データから `○ / △ / ×` で都度集計する。会話のID・本文、Neutral／Default定義、各レイヤーのAccepted画像、会話・戦闘からの表情／衣装参照も個別行で確認でき、クリックすると対応する編集タブへ移動する。未完成のみ表示と再集計に対応する
