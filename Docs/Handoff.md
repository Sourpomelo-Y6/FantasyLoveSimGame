...
- 通常会話の分類ルールは `Docs/ConversationClassificationRules.md` を正本とする。categoryはUnityの `ConversationGenre` に合わせて `Daily / Food / Adventure / Love` の4値を使い、各ジャンルへpriority 0・条件なし・反復可能なフォールバックを最低1件置く。IDは `Conv_<Genre>_<Context>_<NN>` とし、本番公開後は変更しない。TestHeroineは各ジャンルに好感度200～599の親しみ会話と600～9999の信頼以降会話を追加済み。さらに夏昼・冬夜・晴天朝・嵐・雪・雨夜を扱うpriority 200の条件付き会話を用意し、4ジャンルすべてで時間帯・季節・天候のいずれかが反映される
