...
- AssetToolの `制作状況` タブは、現在選択中のヒロインについて基本情報、戦闘メッセージ、訓練画像、会話、表情、衣装、戦闘スキル、スキルツリーを既存データから `○ / △ / ×` で都度集計する。戦闘スキルの設定値と、ツリーノードのルート・ID・前提・付与スキル・解放訓練参照も個別に確認でき、クリックすると基本情報内の編集欄へ移動する。未完成のみ表示と再集計に対応する
