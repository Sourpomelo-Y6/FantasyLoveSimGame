...
## 2026-07-31 条件付き訓練のデータ基盤

- `TrainingData` に調子別の表示・実行条件、一回限定、前提完了条件を追加
- `TrainingAvailabilityEvaluator` で Hidden / Disabled / Available を共通判定
- 成功完了だけを `TrainingCompletionRecord` としてセーブし、中断は完了に含めない
- 一回限定の再実行防止と、前提訓練完了による後続訓練解放に対応
- 既存アセットは条件配列が空のため従来どおり表示・実行可能
- Sceneへの追加UI配置は行わず、既存の訓練ボタンへ実行不可理由を表示
...
- 訓練には `TimeManager.Day` から決まる30日周期の調子を適用する。絶好調・好調10日、普通10日、不調・絶不調10日を固定配置し、普通以外は各ステップの双方のHP消費、好感度、熟練度を補正する。専用セーブ項目は持たず、HP消費は最低1、報酬は最低0を維持する。訓練画面へ任意の `ConditionText` を置くと調子を専用表示でき、未配置時もログで確認できる
...
- 予定の状態はセーブデータに保存される
- 予定の保存と復元は確認済み
- `SaveDataNormalizer` は保存前、通常ロード、プレビュー読込の共通入口でnullコレクション、範囲外数値、重複ID、不正予定、別ヒロインのスキル構成を補正する。旧セーブの移行判定に使う `saveVersion` 自体は変更しない
- `FantasyLoveSim > Validation > Data > Save Data` は `Application.persistentDataPath` の `save.json` / `save_slot_<番号>.json` を検査し、未対応version、ヒロインID欠落、null、重複ID、負数、予定日重複、別ヒロイン設定の混入をConsoleへ表示する
...
- 通常会話の分類ルールは `FantasyLoveSimAssetTool/Docs/ConversationClassificationRules.md` を正本とする。categoryはUnityの `ConversationGenre` に合わせて `Daily / Food / Adventure / Love` の4値を使い、各ジャンルへpriority 0・条件なし・反復可能なフォールバックを最低1件置く。IDは `Conv_<Genre>_<Context>_<NN>` とし、本番公開後は変更しない。TestHeroineは各ジャンルに好感度200～599の親しみ会話と600～9999の信頼以降会話を追加済み。さらに夏昼・冬夜・晴天朝・嵐・雪・雨夜を扱うpriority 200の条件付き会話を用意し、4ジャンルすべてで時間帯・季節・天候のいずれかが反映される
...
- 別リポジトリまたは別フォルダで Stable Diffusion 向けキャラクター素材生成ツールを作る方針。仕様は `FantasyLoveSimAssetTool/Docs/CharacterAssetGenerationToolSpec.md` に整理済み
...
- `FantasyLoveSim > Validation > Data > Action Reaction Data` から、ID・条件衝突・参照先・フォールバック・SimpleAction以外への誤設定を全ヒロイン分検証できる
...
`sprite_layers_export.json` がある場合は、`HeroineLayeredSpriteData.asset` に `Background / BackAccessory / BackHair / CostumeBody / HeadExpression / FrontAccessory / FrontArm / Effect` の8階層をimportする。旧 `BaseBody / Costume / Expression / Accessory` も互換用に読み込める。
...
指定表情がない場合は `Neutral`、指定衣装がない場合は `Default` へ fallback する。新8階層用Imageは未配置でも実行時生成し、8階層データがなければ旧4階層表示を使う。
...
- Unity Editor の `FantasyLoveSim > Validation > Data > Heroine Data` を実行し、ResourcePath、ID 重複、別ヒロイン画像参照、衣装セリフ override の重複 warning を確認する
...
`FantasyLoveSim > Validation > Training Balance Report` は初期HP（主人公100・ヒロイン80）、スキル補正なしで全訓練を終了まで計算し、ステップ数、終了理由、双方のLP消費、同時限界、好感度、熟練度、SPを表示する。現在の基準は、軽い稽古20回・好感度30、実戦形式14回・好感度74、持久訓練18回・好感度28、連携演習20回・好感度32。実戦形式は同時限界2回のボーナス40を含むため、意図的に好感度効率が高い状態である。
訓練画像の自動切替を実装済みで、仕様を `FantasyLoveSimAssetTool/Docs/Extra/TrainingImagePlan.md` に記録している。訓練選択時は現在の訓練画面で `elapsedSteps == 0` なら開始前、1以上なら進行後の画像を使う。ステップごとのLP差分から主人公のみ、ヒロインのみ、双方同時の3状態を判定し、同時状態を優先する。画像はヒロイン別 `HeroineTrainingImageData` に分離し、既存 `heroineImage` を使い、未設定時は現在画像を維持する。TestHeroineは `LightPractice`、`SparringPractice`、`EnduranceTraining` の各5状態、合計15枠を設定済み。`CooperativeDrill` の5状態は未設定で、DefaultHeroineには `HeroineTrainingImageData.asset` がまだない。旧共通LP画像は互換用フォールバックとして利用できる。訓練別画像の優先、共通画像へのフォールバック、未設定時のnull、LP消費4状態は専用EditMode Testで確認する。
訓練中のヒロインセリフは実装済み。画像と同じ `trainingId + visualState` をキーにして訓練ごとに変え、標準15画像枠の各組み合わせへ最低1件ずつ設定できる。各枠には複数候補を登録でき、直前と同じセリフを避けて選択する。セリフデータは画像参照から分離した `HeroineTrainingDialogueData` で管理し、訓練進行時に一度決定した `TrainingVisualState` を画像とセリフへ共有する。TestHeroineには標準15セリフを作成済み。UIは `HeroineNameText` と `TrainingMessageText` を結果ログとは別に配置し、同名子GameObjectの自動検出またはInspector参照を利用する。音声付き候補は `voicedMessages` の本文と `voiceId` の組で設定し、存在する枠では従来の `messages` より優先する。任意の `VoiceReplayButton` は自動検出・コード接続され、訓練終了時には音声を停止する。
AssetToolの訓練画像タブから各枠のセリフ候補と任意のVoice IDを編集し、`training_dialogues_export.json` へ出力できる。音声なし候補は `messages[]`、音声付き候補は `voicedMessages[]` の本文・ID組として同期する。Unityの `HeroineAssetImporter` はこのJSONが存在する場合だけ `HeroineTrainingDialogueData.asset` を生成・更新し、旧ExportでJSONがない場合は既存セリフを維持する。`voicedMessages` キー自体がない旧JSONでは既存音声IDを維持し、新形式の空配列だけを削除として扱う。
...
AssetTool側は `usage = Training`、`Images/Training/`、`training_images_export.json` を追加し、Unity側の `requiredSkillIds`、共通セリフ、`battleSkills`、ヒロイン別スキル・ノードなども段階的に往復対応する。同期範囲、旧JSONで新規フィールドを消さない更新規則、実装順は `FantasyLoveSimAssetTool/Docs/Extra/CurrentFeatureSyncPlan.md` を参照する。
...
訓練数増加時のUIと条件付き訓練は `Docs/TrainingSelectionExpansionPlan.md` に設計をまとめた。スクロール一覧と詳細欄、表示条件と実行条件の分離、絶好調限定、不調時実行不可、一回限定、成功完了による後続訓練解放を段階導入する。既存の訓練回数は中断も数えるため、一回限定とチェーン解放には専用の成功完了記録を使う。
訓練一覧拡張の第1段階として、`TrainingPanel` に一覧選択と開始の分離、実行可能のみのフィルター、実行可能優先・`sortOrder`・表示名・IDによる安定ソート、選択中Outline、説明・消費・報酬・条件の詳細表示を追加した。`TrainingStartButton` が未配置なら従来どおり一覧ボタンで即開始するため、既存Sceneとの互換性を維持する。新UIのScene配置とScrollRect化は手作業が必要。
...
スケジュール拡張のデータ基盤は実装済み。セーブバージョン19から `SaveData.scheduleEntries` を正本とし、`ScheduleEntry` はゲーム内日数、予定種別、`Planned` / `Executed` / `Cancelled`、キャンセル理由を保持する。`ScheduleManager` は日付指定の取得・設定・変更・キャンセル、期間一覧、実行済み化を提供し、編集可能範囲は現在日から30日先まで。既存の今日・明日APIと画面は一覧を参照する互換窓口として残す。バージョン18以前のロード時は旧今日・明日フィールドを現在日と翌日へ移行し、新形式の保存にも移行期間中は旧フィールドを併記する。`SchedulePanel` の週間・月間表示とScene UIは実装・動作確認済み。7日枠とテンプレートから生成する30日枠を切り替え、期間移動、選択日詳細、状態色分け、選択日への設定・変更・キャンセルを共通処理で扱う。月間グリッドは先頭空白を含む最大36セルを生成し、日曜を左端、土曜を右端に揃える。`WeeklySelectedDayAnchor` / `MonthlySelectedDayAnchor` のRectTransform設定を表示切替時に `SelectedDayArea` へコピーし、表示ごとに詳細欄の位置と大きさを変えられる。UI参照はInspector割り当てに加えて名前からの自動検出も行う。
スケジュールテンプレート機能も実装済み。`ScheduleTemplateManager` は7日・30日の予定種別を `Application.persistentDataPath/schedule_templates.json` へ保存し、セーブスロット間で共有する。一覧、新規保存、ID指定の上書き・削除、適用前プレビュー、選択日からの一括適用を提供する。状態は保存せず、適用時は予定を `Planned` として扱う。空の予定も期間内の1枠として適用件数へ含め、適用先に既存予定がある場合は上書き許可時だけ削除する。既存予定の上書き可否を指定でき、過去、編集可能範囲外、実行済みの日はスキップする。`ScheduleTemplatePanelController` は配置済みのUIを名前から自動検出し、一覧生成、選択、新規保存、確認付き上書き・削除、プレビュー後の適用、結果表示、カレンダー更新を制御する。`SchedulePanel` は `OpenTemplateButton` から表示期間の先頭日を保存元、現在の選択日を適用先として別々に渡す。ControllerがSceneに未配置でも `ScheduleTemplateManager` と同じGameObjectへ実行時に追加する。
スキルポイント基盤は実装済み。主人公とヒロインのポイントは `SaveData.playerSkillPoints` / `heroineSkillPoints` に別々に保存する。完了かつ非中断の訓練だけが `TrainingData.playerSkillPointReward` / `heroineSkillPointReward` を付与し、軽い稽古は双方 1、実戦形式と持久訓練は双方 2。訓練結果メッセージと `StatusProgressPanel` の全体表示に獲得量・現在値を表示する。参照は `GameManager.PlayerSkillPoints` / `HeroineSkillPoints`、安全な消費は `TrySpendPlayerSkillPoints(...)` / `TrySpendHeroineSkillPoints(...)` を使う。消費成功時は状態詳細表示も更新する。旧セーブは 0。`EnemyData.playerSkillPointReward` / `heroineSkillPointReward` は予定探索の勝利時だけ付与し、敗北、逃走、デバッグ戦闘は除外する。森スライムと湖の精霊は双方1、洞窟コウモリは双方4。`BattlePanel` とフォールバック簡易戦闘の両方で加算し、結果メッセージとログに獲得量を表示する。
...
`SkillTreeDataValidator` はノード ID、前提関係、循環参照、所有者・対象ヒロイン、主人公・ヒロインスキル、条件対象 ID、条件重複、座標重複を検証する。Unity Editor の `FantasyLoveSim > Validation > Data > Skill Tree Data` で全ノードを検証でき、Editor Play と Development Build の開始時にも一度実行する。警告は `[SkillTreeValidation]` で始まるため Console で絞り込み可能。通常リリースビルドでは起動時検証を呼ばない。
...
新規会話IDは `Conv_Daily_Morning_01` のように `Conv_<Genre>_<Context>_<NN>` を使い、`showOnce` は一度見せたら再出現させたくない会話だけに使う。既存IDは表示履歴との互換性のため変更しない。
...
- `pages`: 複数ページの話者、任意話者名、本文、表情ID、スチルID・画像。1件以上ある場合はこちらを使用する
...
`FantasyLoveSim > Validation > Data > Ending Data` では、ID・本文・好感度範囲、衣装とイベントの参照、フォールバック不足、同時成立時の選択曖昧さを全ヒロイン分検証できます。画像が未完成の場合は `stillSprite` を空のまま運用できます。
...
`EndingManager` は `EndingData.pages` を順番に表示する。`pages` が空なら旧 `message` / `stillSprite` をSystem話者の1ページとして表示するため、既存アセットはそのまま利用できる。`nextButton` がある場合は最終ページまでNextを表示し、最終ページでTitleボタンへ切り替える。`speakerNameText`、`nextButton`、`layeredSpriteView` は未設定でも例外を出さない。

EndingSceneへ手作業で追加するUI:

- `SpeakerNameText`: `TextMeshProUGUI`。本文の上に配置し、`EndingManager.speakerNameText` へ割り当てる
- `NextButton`: `Button`。Titleボタンと同じ領域またはその左側へ配置し、`EndingManager.nextButton` へ割り当てる。OnClickはコードで登録するためInspectorのイベント設定は不要
- `HeroineLayeredSpriteView`: 既存MainSceneと同じ `BaseBodyImage` / `CostumeImage` / `ExpressionImage` / `AccessoryImage` 子階層を持つ立ち絵ルートを配置し、`EndingManager.layeredSpriteView` へ割り当てる

立ち絵UIをまだ配置しない場合でも本文とページ送りは動作する。スチルがあるページでは `stillImage`、表情IDがあるページでは配置済み立ち絵を更新する。
...
再表示せず、アプリ再起動時に再表示する。表示済み状態はセーブデータへ保存しない。
画像・音声参照が未設定でも例外なく無音・代替表示で動作させる。
...
### Heroine3 非画像データの整備状況

Heroine3（クーデリア）は、メニュー13件、会話・イベント65件、戦闘スキル3件、
訓練4件、訓練セリフ20枠、スキルツリーノード7件、訓練スキル3件、
戦闘結果イベント4件を整備済み。画像は引き続き別工程とする。
AssetTool作業データに残っていたTestHeroine由来の注記は、Heroine3用の注記へ整理した。
...
通常会話は `FantasyLoveSim > Validation > Data > Conversation Data` から全ヒロイン分を検証できる。
