...
訓練中のヒロインセリフは、画像と同じ `trainingId + visualState` をキーにして訓練ごとに変える。標準15画像枠の各組み合わせに最低1件ずつ、合計15種類以上を設定でき、各枠には複数候補も登録できる設計とする。セリフデータは画像参照から分離した `HeroineTrainingDialogueData` で管理し、訓練進行時に一度決定した `TrainingVisualState` を画像とセリフへ共有する。UIではヒロイン名・セリフ欄を結果ログから分離する。
