...
スキル取得と装備は、`StatusDetailPanel` へ直接追加せず、別の `SkillPanel` として扱う。`SkillPanel` は実装済みで、全スキルの未取得/取得済み状態、説明、解放条件を表示する。通常一覧の入口はヒロイン別 `Actions/SkillAction.asset` と `ActionExecutionType.OpenSkillPanel` であり、既存の行動メニューから開く。戦闘中は `BattlePanel` の Skill ボタンが解放済み戦闘スキルの選択を開き、`PowerStrike`（Damage）、`GuardStance`（Guard）、`FirstAid`（Heal）を実行できる。`SkillPanel` の Canvas 配下への手動配置と一覧 UI 参照の Inspector 割り当ては必要。`GameManager.skillPanel` が未割り当てでも、非アクティブを含むシーン内の `SkillPanel` を自動検索する。ヒロイン用スキル、カテゴリタブ、装備枠、Buff/Debuff の戦闘効果は後回しにする。
