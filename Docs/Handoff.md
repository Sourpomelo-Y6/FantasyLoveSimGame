...
訓練終了結果は `TrainingResult` で扱う。`TrainingPanel` は HP/LP 終了時、途中終了時、進行中に閉じた時に一度だけ `GameManager.OnTrainingPanelResult(...)` へ結果を通知する。現段階では `GameManager` は Debug.Log に結果を出すだけで、報酬や熟練度にはまだ反映しない。
