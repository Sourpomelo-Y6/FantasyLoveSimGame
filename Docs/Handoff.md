...
訓練終了結果は `TrainingResult` で扱う。`TrainingPanel` は HP/LP 終了時、途中終了時、進行中に閉じた時に一度だけ `GameManager.OnTrainingPanelResult(...)` へ結果を通知する。進行ボタンで有効な 1 ステップを進めるたびに `TrainingData.affectionRewardPerStep` を累積し、初期訓練はすべて `1`。中断してもステップ分は好感度へ反映する。完了かつ非中断の場合だけ `TrainingData.affectionReward` と同時 0 ボーナスを追加する。結果は `ShowSystemMessage(...)` で画面に表示し、メッセージログにも残す。1 ステップ以上進めた訓練は、完了/中断に関わらず時間を 1 段階進める。訓練熟練度は `SaveData.trainingProficiencies` に `trainingId` ごとの値として保存し、完了時のみ `trainingProficiencyReward` を加算する。報酬反映後は `SkillData` の好感度、日数、訓練熟練度、前提スキル条件を確認し、達成済みスキルを自動解放する。`TrainingPanel` は訓練ボタンと選択中タイトルに現在の熟練度を表示する。
...
好感度尺度は整数 `0〜9999`。既存の値・増減・条件は10倍へ移行し、従来の100相当は1000、ランク境界は200、400、600、800、1000とする。`HeroineStatus.maxAffection = 9999` と `endingUnlockAffection = 1000` を分離済み。旧 `maxAffection = 100` は通常コンテンツの終端でもあったため、移行後の会話・行動の既定上限は9999にして1000以降も利用可能にする。1000以降は単純な累積値として扱える。`SaveData.saveVersion` は10で、旧好感度尺度とのセーブ互換性は保証しない。
...
初期確認用のスキルデータは `Assets/Resources/Skills` に `PowerStrike`、`GuardStance`、`FirstAid` として追加済み。解放条件は順に `LightPractice` 熟練度 3、`EnduranceTraining` 熟練度 3、`SparringPractice` 熟練度 5 とする。
