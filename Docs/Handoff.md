...
訓練数増加時のUIと条件付き訓練は未実装で、`Docs/TrainingSelectionExpansionPlan.md` に設計をまとめた。スクロール一覧と詳細欄、表示条件と実行条件の分離、絶好調限定、不調時実行不可、一回限定、成功完了による後続訓練解放を段階導入する。既存の訓練回数は中断も数えるため、一回限定とチェーン解放には専用の成功完了記録を使う。
