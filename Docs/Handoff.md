...
- 訓練には `TimeManager.Day` から決まる30日周期の調子を適用する。絶好調・好調15日と不調・絶不調15日を固定配置し、各ステップの双方のHP消費、好感度、熟練度を補正する。専用セーブ項目は持たず、HP消費は最低1、報酬は最低0を維持する。訓練画面へ任意の `ConditionText` を置くと調子を専用表示でき、未配置時もログで確認できる
