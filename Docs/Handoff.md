...
- AssetToolの `制作状況` タブは、現在選択中のヒロインについて基本情報、戦闘メッセージ、訓練画像を既存データから `○ / △ / ×` で都度集計する。基本項目、必須勝敗メッセージ、各訓練の5画像状態を個別行で確認でき、カテゴリまたは詳細行をクリックすると対応する編集タブへ移動する。未完成のみ表示と再集計に対応する
