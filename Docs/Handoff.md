...
`StatusProgressPanel` は実装済みで、全体、カテゴリー別、訓練別、敵別の表示を切り替える。`StatusDetailPanel.progressButton` から開き、`progressPanel` は状態詳細の子に配置した場合は自動検出可能。Unity 上ではパネルルート、タイトル Text、スクロール可能な本文 Text、全体・カテゴリー・訓練・敵 Button、閉じる Button を配置し、`StatusProgressPanel` の各参照と `StatusDetailPanel.progressButton` / `progressPanel` を割り当てる。Scene への UI 配置は未実装。
