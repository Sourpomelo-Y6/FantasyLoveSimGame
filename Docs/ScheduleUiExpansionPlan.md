...
月間表示コードも実装済み。`WeeklyViewButton` / `MonthlyViewButton` で表示を切り替え、`MonthGrid` の非アクティブな `ScheduleDayCell` テンプレート1個から30日枠を生成する。日付枠は週間と同じ選択日、予定状態、色、編集可否を使い、表示を切り替えても選択日を維持する。月間の現在期間は現在日から29日先までを表示し、前期間では過去30日を閲覧できる。次期間への移動は現在日開始までに制限し、編集可能範囲外の予定を増やさない。
