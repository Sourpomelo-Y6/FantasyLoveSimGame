...
最初は `BattlePanel` を流用し、敵枠にヒロインまたは主人公のコピーを入れる形で検証する。
模擬戦闘中の HP は通常戦闘と同じくコピーを使い、終了後に本来の HP を直接減らさない。
...
スキルデータには、`skillId`、表示名、カテゴリ、説明、消費コスト、対象、効果種別、威力または回復量、解放条件、使用可能な戦闘種別を持たせる案にする。
...
実装順は、まずスキルデータ定義と表示だけを作り、次に戦闘用スキルを `BattlePanel` へ接続し、その後に模擬戦闘と訓練用スキルを扱う。
