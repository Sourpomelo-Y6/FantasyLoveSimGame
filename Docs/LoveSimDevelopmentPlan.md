...
1. 行動反応と会話データの追加（TestHeroineは4ジャンルの無条件フォールバック、好感度200～599と600～9999の反復可能会話、時間帯・季節・天候に応じた条件付き会話を追加済み）
2. エンディングデータと条件分岐パターンの追加（TestHeroineの3分岐、複数ページ・話者・表情、検証ツールを実装済み）
3. 立ち絵変更と表情差分
4. セーブ/ロード回帰確認とUI補強（正規化、保存データ検証、主要状態のJSON往復テストを実装済み）
5. 訓練、戦闘、ショップのデータ追加と調整
6. タイトル画面用画像とレイアウト、フィクション表記、BGM・SE、音声データなしで動くボイス再生基盤の追加（音響基盤とフィクション表記は実装済み。詳細は `Docs/TitleAndAudioPresentationPlan.md`）
...
プレイヤーとヒロインそれぞれの詳細ステータス画面は閲覧専用として維持する。
能力取得はスキルツリーへ統合済みで、衣装確認モードの利用可否も取得済み主人公ノードから再構築する。詳細は `Docs/StatusAndAchievementUiReorganizationPlan.md` を参照する。
メイン画面の主人公・ヒロイン HP 表示は、まずテキストで現在値と最大値を表示する。
HP バーやゲージ色変更などの視覚的な強化は、戦闘の基本導線や結果反映が落ち着いてから後回しで検討する。
状態画面ではHP、MP、戦闘能力、装備・有効化中スキル、所持品、解放衣装、好感度、現在衣装、衣装確認モードの解放状況と現在設定を対象キャラクターに応じて表示する。スキルは表示名を使い、空欄は「なし」とする。取得や使用設定は状態画面では行わない。
状態本文は縦ScrollRect内へ置き、TMPの表示更新後に `preferredHeight` からContentの高さを再計算する。短い本文はViewportの高さを下限とし、画面を開いたときと主人公・ヒロイン切り替え時は表示位置を先頭へ戻す。
...
│   └── SeasonText
├── SchedulePanel
├── StatusDetailPanel（主人公・ヒロインの閲覧切り替え）
├── StatusProgressPanel（実績の全体・カテゴリー・訓練・敵表示）
├── DialoguePanel
│   ├── SpeakerNameText
│   ├── DialogueText
│   ├── ChoiceButtonArea
...
| CaveBat | 55/55 | 20 | 3 | 8 | - | 報酬 450、勝利時好感度 +30 |
...
簡易戦闘計算は副作用のない `SimpleBattleSimulator` に分離し、ゲーム本体とEditMode Testで共用する。`FantasyLoveSim > Validation > Battle Balance Report` では初期ステータスによる全敵の単独・同行結果をConsoleへ出力する。現在の回帰基準は、ForestSlimeが単独3ターン・主人公被害2、CaveBatが単独7ターンで敗北・主人公被害100／同行5ターンで勝利・主人公被害45・ヒロイン被害32、LakeSpiritが単独5ターン・主人公被害4とする。敵パラメータを意図的に調整した場合は、この基準とテストも同時に更新する。
...
現在は `Assets/Resources/ShopItems/SpringOutfitItem.asset`、`SummerOutfitItem.asset`、`AutumnOutfitItem.asset`、`WinterOutfitItem.asset` を季節衣装商品として定義済み。価格は200G、300G、450G、600G。夏は7日目、秋は好感度200と春購入済み、冬は14日目と秋購入済みを条件にし、初期1000Gだけで全衣装を購入できない進行とする。
`Assets/Resources/ShopItems/DuoShoppingCatalog.asset` は季節衣装4件とHP・MPポーションの計6件を持つ。
...
`FantasyLoveSim > Validation > Shop Balance Report` は全商品の価格・購入可能日・必要好感度・前提商品を表示し、季節衣装の合計価格と初期所持金で全購入できるかを確認する。価格や条件を調整した場合はショップ進行のEditMode Testも同時に更新する。
...
ショップの商品一覧と詳細表示を分けるコードは実装済み。商品ボタンを選択すると商品名、種別、価格、説明、解放内容を含む効果、購入条件、現在の購入可否を表示し、`PurchaseButton`で確定する。購入済み・条件未達・所持金不足の商品も詳細確認でき、購入後は一覧、選択状態、所持金、詳細欄を更新する。`ShopItemData.description` が空なら回復量や解放衣装から説明を自動生成する。新しいScene UIを未配置の場合は従来の即購入を維持する。`ShopItemList` のScroll View化と、すべて・衣装・消耗品・その他のカテゴリ切り替えも実装済み。
消耗品は商品一覧と `OwnedQuantityText` に現在の所持数を表示し、購入後に即時更新する。衣装など一度だけ購入する商品は、詳細欄に未所持または購入済みを表示する。
戦闘消耗品は `maxOwnedQuantity` が1以上なら最大所持数を適用し、0なら無制限とする。上限時は購入不可にし、戦闘で使用して数量が下がると再購入できる。HP・MPポーションは各9個を上限とし、既存セーブの超過分は維持する。
...
ソロ探索から帰宅した後のヒロイン反応は、戦闘直後イベントと分離した `SoloReturnReactionData` で扱う。ヒロインプロフィールの `soloReturnReactionResourcePath` から勝利・敗北・撤退と `battleContextId` に合うデータを読み、共通の戦闘ログ・戦闘後イベントより後へ追加する。帰還後反応ではヒロインの本文、Voice ID、表情、スチル、表示方式を利用でき、未設定なら何も追加しない。TestHeroineの旧ヒロイン別 `SoloVictory` / `SoloDefeat` はこのフォルダへ移し、単独予定の探索結果本文は予定話者へ変更する。
...
訓練数の増加に備え、一覧選択と開始の分離、詳細表示、実行可能フィルター、
安定ソートのコード接続は実装済み。Sceneのスクロール一覧配置と、調子限定、一回限定、
成功完了による後続訓練解放は今後の拡張とする。表示条件と実行条件を分け、
...
訓練の調子機能は30日周期とし、`TimeManager.Day` から周期内の日を決定する。好調側10日、補正なしの普通10日、不調側10日を固定配置し、普通以外はバフまたはデバフを全訓練の各ステップへ適用する。補正は、絶好調が双方のHP消費-2・好感度+1・熟練度+2、好調が双方のHP消費-1・熟練度+1、普通が全項目±0、不調が双方のHP消費+1・熟練度-1、絶不調が双方のHP消費+2・好感度-1・熟練度-2とする。スキル補正を先に、調子補正を後に適用し、元のHP消費が1以上なら最終消費は最低1、好感度・熟練度報酬は最低0とする。日付から再現できるため専用のセーブ項目は追加せず、ロード後も保存済み日付と同じ調子になる。`TrainingPanel` は当日の調子、周期日、補正内容をログへ表示し、任意の `ConditionText` が配置されていれば専用欄にも表示する。
...
訓練の基礎バランスは `TrainingBalanceCalculator` で副作用なく計算し、`FantasyLoveSim > Validation > Training Balance Report` とEditMode Testで共有する。初期HP・スキル補正なしの基準は、軽い稽古が20回・好感度30・熟練度21、実戦形式が14回・好感度74・熟練度16、持久訓練が18回・好感度28・熟練度21、連携演習が20回・好感度32・熟練度23。実戦形式の好感度74には同時限界2回のボーナス40が含まれる。報酬を調整するときは回帰テストとこの基準を同時に更新する。
...
画面機能や衣装確認モードなどの解放専用効果も `SkillTreeNodeData` に持たせ、取得状態の正本をスキルツリーへ統一する。
スキルは次のカテゴリに分ける。
...
- セーブ/ロード回帰確認では、好感度、日付、時間帯、曜日、季節、天気、現在衣装、衣装評価、今日/明日の予定、当日予定イベントの発動済み状態、スキル取得状態、衣装確認モードの利用可否と現在の使用モードを確認する。旧能力取得状態は移行期間だけ確認する
...
8. 衣装確認モードの解放条件は取得済み主人公スキルツリーから再構築し、`GameManager.playerOutfitPromptAbilities` には現在の使用モードだけを保持する
