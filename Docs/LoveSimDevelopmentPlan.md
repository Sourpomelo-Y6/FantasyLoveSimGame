...
プレイヤー能力とヒロイン能力は表示枠と保存先を分けているが、能力項目自体はまだ衣装確認モード中心なので、今後は `StatusDetailRole` ごとに一覧を分離する。
入口は `StatusDetailAction` を行動一覧に置くと、既存の行動導線に自然に混ぜられます。
ただし、`StatusDetailPanel` の現在の実装はランタイム生成が多いので、最終的には Unity の Hierarchy 上に手で配置して、`GameManager` から参照する形へ寄せる。
置き換えの優先度は、`panelRoot`、ラベル群、タブボタン、能力リスト、解放パネル、`GameManager.EnsureStatusDetailPanel()` の順で進める。
手配線が完了したら、次の調整対象はメイン画面のボタン配置に移る。
