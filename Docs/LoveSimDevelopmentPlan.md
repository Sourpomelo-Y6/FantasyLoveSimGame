...
1. プレイヤーとヒロインの戦闘・買い物用ステータスを追加する。最小項目は HP、最大 HP、攻撃、防御、素早さ、所持金
2. 追加したステータスを `StatusDetailPanel` に表示する
3. 追加したステータスを `SaveData` に保存し、セーブ/ロードで復元する
4. 所持金を増減するテスト処理を用意する
5. `DuoShopping` などの予定から簡易買い物イベントを起動する
6. 買い物でアイテムまたは衣装を購入し、所持金消費と解放状態を反映する
