...
プレイヤーとヒロインそれぞれの詳細ステータス画面は維持するが、以下の能力取得に関する記述は移行前の現行実装を示す。新方針では能力取得導線を撤去する。
実装上は `StatusDetailPanel` で詳細表示と能力獲得画面の切り替えをまとめていたが、能力取得はスキルツリーへ統合する方針へ変更する。状態画面は閲覧専用とし、移行計画は `Docs/StatusAndAchievementUiReorganizationPlan.md` を参照する。
移行完了後はスキルツリーノードの取得状態を保存し、衣装確認モードの利用可否を再構築する。
...
旧 `StatusAbilityData` と能力一覧は段階的に廃止する。衣装確認モードの解放は主人公スキルツリーへ移し、状態画面では現在のモードと使用中スキルの要約だけを表示する。
...
8. 衣装確認モードの解放条件は取得済み主人公スキルツリーから再構築し、現在の使用モードだけを設定値として保持する。移行中は `GameManager.playerOutfitPromptAbilities` を互換用に残す
