...
訓練スキルの有効状態管理は実装済み。主人公は `SaveData.activePlayerTrainingSkillIds`、ヒロインは `heroineId` ごとの `SaveData.heroineTrainingSkillActivations` に保存し、取得済みで有効な `SkillData` だけを維持する。スキルツリーの取得済み訓練ノードでは既存ボタンを「有効にする／無効にする」として使い、有効数に上限は設けない。セーブバージョン15以前は取得済み訓練スキルをすべて自動的に有効化し、バージョン16以降は無効にした状態も維持する。初期確認用として主人公の「深呼吸」と、DefaultHeroine / TestHeroine の「呼吸合わせ」を追加する。現段階では有効状態の管理までとし、HP消費・好感度・熟練度への実効果は次段階で接続する。
