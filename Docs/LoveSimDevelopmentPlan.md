...
二人で買い物に行く予定や行動では、将来的にアイテムや衣服を購入できる機能を追加する。
`DuoShopping` は単なるスチル表示イベントではなく、店の種類、商品一覧、所持金、購入結果、衣装解放を扱う専用フローへ拡張する。
購入した衣服は `OutfitData` の解放状態と連動させ、アイテムは将来のプレイヤー/ヒロイン状態や戦闘、イベント条件に使えるようにする。

買い物以外のお出かけ、たとえば森、洞窟、湖などは、現在のスチル表示中心の予定イベントから、RPG でよくある探索・戦闘要素を持つ行動へ拡張する。
最初は簡易的な戦闘でよく、敵、HP、攻撃、防御、素早さ、報酬、敗北時の処理を最小単位として実装する。
探索や戦闘の結果は、好感度、入手アイテム、衣装解放、イベント発生条件、エンディング条件へつなげられるようにする。

ステータス画面には、買い物や戦闘で使うパラメータを表示する。
プレイヤーとヒロインの両方に、戦闘用パラメータとして HP、攻撃、防御、素早さ、所持金、装備または着用衣装による補正を追加する予定。
実装時は `SaveData` に保存する値と、ScriptableObject で定義する初期値/成長値を分ける。
UI は既存の `StatusDetailPanel` を拡張し、能力項目とは別に基本能力、戦闘能力、所持品または購入済み衣服を確認できる欄を追加する。

少しずつ進める場合は、戦闘 UI や敵 AI からではなく、買い物・探索・戦闘で共通利用する基礎ステータスから着手する。
最初の到達点は「ステータス詳細画面で戦闘用パラメータが見えて、セーブ/ロードで復元される」状態にする。
...
5. `DuoShopping` などの予定から簡易買い物イベントを起動する。実装済み。DuoShopping 実行時にテスト費用を消費する
6. 買い物でアイテムまたは衣装を購入し、所持金消費と解放状態を反映する。`ShopCatalogData` と `ShopItemData` のテスト商品 `ShoppingTestItem_01` の購入済み保存と春夏秋冬衣装解放まで実装済み
...
`ShoppingTestItem_01` 購入時は商品データの解放衣装 ID を `SaveData.unlockedOutfitIds` に保存する。
...
最初のデータ化単位は、`ScheduleType`、勝敗、Solo/Duo の組み合わせとする。
データ化前の整理として、固定文生成は勝敗と Solo/Duo から内部イベント種別を解決してから文面を返す形にする。
`BattleResultEventData` は、`BattleResultEventType`、メッセージ本文、将来用の `stillId`、`affectionChange`、`unlockedOutfitIds` を持つ。
...
初期データとして `SoloVictory`、`DuoVictory`、`SoloDefeat`、`DuoDefeat` を用意し、既存固定文と同じ文面を設定する。
...
現時点では専用戦闘 UI は作らず、探索結果メッセージの後続ページとして敵名、勝敗、ターン数、被ダメージ、現在 HP、報酬、好感度変化を表示する。
