...
訓練の調子機能は30日周期とし、`TimeManager.Day` から周期内の日を決定する。好調側15日と不調側15日を固定配置し、毎日いずれかのバフまたはデバフを全訓練の各ステップへ適用する。補正は、絶好調が双方のHP消費-2・好感度+1・熟練度+2、好調が双方のHP消費-1・熟練度+1、不調が双方のHP消費+1・熟練度-1、絶不調が双方のHP消費+2・好感度-1・熟練度-2とする。スキル補正を先に、調子補正を後に適用し、元のHP消費が1以上なら最終消費は最低1、好感度・熟練度報酬は最低0とする。日付から再現できるため専用のセーブ項目は追加せず、ロード後も保存済み日付と同じ調子になる。`TrainingPanel` は当日の調子、周期日、補正内容をログへ表示し、任意の `ConditionText` が配置されていれば専用欄にも表示する。
