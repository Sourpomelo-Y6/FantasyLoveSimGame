...
`EndingScene` には `EndingManager` を配置し、`Assets/Resources/Endings/` の `EndingData` からエンディング用テキストとスチルを表示した後に `TitleButton` から `TitleScene` へ戻れるようにする。
...
将来は画面内、またはメッセージ表示ウィンドウをクリックしても次のメッセージへ進めるようにする。
ただし、選択肢、Save/Load、メッセージログ、ステータス詳細などの UI 操作と競合しないよう、まずはメッセージ表示ウィンドウへのクリックを進行操作として扱うのが安全。
