...
6. 買い物でアイテムまたは衣装を購入し、所持金消費と解放状態を反映する。`ShopItemData` のテスト商品 `ShoppingTestItem_01` の購入済み保存と春夏秋冬衣装解放まで実装済み
7. 森、洞窟、湖などの簡易探索フローを追加する
8. 敵、HP、攻撃、防御、素早さ、報酬、敗北時処理を持つ簡易戦闘を追加する
...
テスト商品は `ShopItemData` で定義し、`GameManager.duoShoppingShopItem` に設定する。
現在は `Assets/Resources/ShopItems/ShoppingTestItem_01.asset` が価格 100、解放衣装 `Spring` / `Summer` / `Autumn` / `Winter` を持つ。
簡易買い物イベントとして、`DuoShopping` 予定の実行時に商品データの価格を消費する。
所持金が足りない場合は、買い物できなかった旨を予定イベント本文に追記する。
`GameManager.duoShoppingShopItem` が未設定の場合は、従来の `duoShoppingTestItemId` / `duoShoppingTestCost` / `duoShoppingUnlockedOutfitIds` にフォールバックする。
