...
6. 買い物でアイテムまたは衣装を購入し、所持金消費と解放状態を反映する。固定テスト商品 `ShoppingTestItem_01` の購入済み保存と春夏秋冬衣装解放まで実装済み
...
簡易買い物イベントとして、`DuoShopping` 予定の実行時に `GameManager.duoShoppingTestCost` の金額を消費する。
デフォルトは 100。所持金が足りない場合は、買い物できなかった旨を予定イベント本文に追記する。
固定テスト商品として `GameManager.duoShoppingTestItemId`、デフォルト `ShoppingTestItem_01` を購入済みに登録する。
