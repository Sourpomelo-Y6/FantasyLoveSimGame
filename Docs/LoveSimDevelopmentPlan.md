...
プレイヤー能力とヒロイン能力は表示枠と保存先を分け、`StatusDetailPanel.playerAbilityKinds` と `StatusDetailPanel.heroineAbilityKinds` で表示する能力一覧も分ける。
初期値では両方に同じ衣装確認能力を入れて互換性を保つが、今後は `StatusDetailRole` ごとに別能力を追加する。
